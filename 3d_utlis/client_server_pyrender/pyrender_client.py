...
def send_render_request(room_name, step_number, ground_truth):
    """Send a rendering request to the Pyrender server"""
    ...
    try:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            # Get the response
            # Read the response in chunks as it might be larger than 4096
            response_data = b""
            while True:
                chunk = s.recv(4096)
                if not chunk:
        ...
        print("Error: Could not connect to the Pyrender server. Make sure it's running.")
    except Exception as e:
        print(f"Error: {str(e)}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Send rendering requests to the Pyrender server")
